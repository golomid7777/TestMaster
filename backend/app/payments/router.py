...
def create_payment(
    ...
    try:
        ...
        amount_rubles = f"{topic.price_kopecks / 100:.2f}"
        ...
        payload = {
            ...
            "description": f"Доступ TestMaster: {topic.name}",
            "metadata": {
        ...
        if not yookassa_payment_id or not confirmation_url:
            raise HTTPException(
                status_code=502,
                detail="ЮKassa не вернула ссылку на оплату"
            )
        ...
        return {
            ...
            "amount_kopecks": payment_row.amount_kopecks,
            "access_minutes": payment_row.access_minutes,
...
def yookassa_webhook(data: dict):
    ...
    # Повторно получаем платёж напрямую из ЮKassa.
    # Не доверяем только данным входящего webhook.
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail="Не удалось проверить платёж в ЮKassa"
        )

    if response.status_code != 200:
        raise HTTPException(
            status_code=502,
            detail="ЮKassa не подтвердила платёж"
        )
    ...
    try:
        ...
        if not payment:
            raise HTTPException(
                status_code=404,
                detail="Платёж TestMaster не найден"
            )

        # Webhook может прийти повторно.
        # Если платёж уже обработан — второй доступ не выдаём.
        ...
        expected_amount = f"{payment.amount_kopecks / 100:.2f}"

        if (
            amount.get("currency") != "RUB"
            or amount.get("value") != expected_amount
        ):
            raise HTTPException(
                status_code=400,
                detail="Сумма платежа не совпадает"
            )
        ...
        current_access = (
            ...
            .filter(
                TopicAccess.user_id == payment.user_id,
                TopicAccess.topic_id == payment.topic_id,
